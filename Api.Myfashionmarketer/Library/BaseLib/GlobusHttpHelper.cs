...
namespace BaseLib
{
    public class GlobusHttpHelper
    {
       ...
       public static List<string> LstPicUrlsGroupCampaignManager
        ...
        public static string Get_AjaxpipeToken(string pgSrc)
        {
            try
            {
                string[] tokens = pgSrc.Split(':');
                string ajaxToken = getBetween(tokens[1], "\"", "\"");
                return ajaxToken;
            }
            catch (Exception ex)
            {
        ...
        public string getHtmlfromUrl(Uri url)
        {
            ...
            try
            {
                ...
                gRequest.UserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36";
               ...
               // gRequest.Headers["Accept-Charset"] = "ISO-8859-1,utf-8;q=0.7,*;q=0.7";
                gRequest.Headers["Accept-Language"] = "en-US,en;q=0.8";//Accept-Language: 
               ...
               // StreamReader reader = new StreamReader(gResponse.GetResponseStream(), Encoding.ASCII);
                StreamReader reader = new StreamReader(gResponse.GetResponseStream(), Encoding.UTF8);
        ...
        public bool MultiPartImageUpload(ref GlobusHttpHelper httpHelper, string Username, string Password, string localImagePath, string proxyAddress, string proxyPort, string proxyUsername, string proxyPassword)
        {
            ...
            {
              Console.WriteLine("Please Check The Account : " + Username);
                Console.WriteLine("Please Check The Account : " + Username);
        ...
        public string HttpUploadPictureForWallNew(ref GlobusHttpHelper HttpHelper, string userid, string url, string paramName, string contentType, string localImagePath, NameValueCollection nvc, string proxyAddress, int proxyPort, string proxyUsername, string proxyPassword, string picfilepath)
        {
        ...
        public string HttpUploadPictureForWallNewFinal(ref GlobusHttpHelper HttpHelper, string userid, string url, string paramName, string contentType, string localImagePath, NameValueCollection nvc, string proxyAddress, int proxyPort, string proxyUsername, string proxyPassword, string picfilepath)
        {
            string responseStr = string.Empty;
        ...
        startAgain:
            ...
            try
            {
                ...
                if (response.Contains("post this because it has a blocked link"))
                {
                    try
                    {
                      Console.WriteLine("-------blocked link-------");
                        return false;
