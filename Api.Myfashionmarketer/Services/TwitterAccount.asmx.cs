...
namespace Api.Myfashionmarketer.Models
{
    ...
    public class TwitterAccount : System.Web.Services.WebService
    {
        ...
        public string getAllTwitterAccountsOfUser(string UserId)
        {
            try
            {
                List<Domain.Myfashion.Domain.TwitterAccount> objTwitterAccount = objTwitterAccountRepository.getAllTwitterAccountsOfUser(Guid.Parse(UserId));
                return new JavaScriptSerializer().Serialize(objTwitterAccount);
            }
            catch (Exception ex)
            {
        ...
        public string GetAllAccountDetailsByProfileId(string profileid, string userid)
        {
            try
            {
                ...
                List<Domain.Myfashion.Domain.TwitterAccount> lstAllTwtAccount = objTwitterAccountRepository.getAllAccountDetail(profileid, Guid.Parse(userid));
