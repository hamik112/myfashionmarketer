...
namespace Api.Myfashionmarketer.Models
{
    ...
    public class Twitter : System.Web.Services.WebService
    {
        ...
        Domain.Myfashion.Domain.TwitterStats objTwitterStats;
        TwitterMessageRepository objTwitterMessageRepository = new TwitterMessageRepository();
        Domain.Myfashion.Domain.TwitterMessage objTwitterMessage;
        TwitterDirectMessageRepository objTwitterDirectMessageRepository = new TwitterDirectMessageRepository();
        Domain.Myfashion.Domain.TwitterDirectMessages objTwitterDirectMessages;
        ScheduledMessageRepository objScheduledMessageRepository = new ScheduledMessageRepository();
        Domain.Myfashion.Domain.ScheduledMessage objScheduledMessage;
        ...
        public void getUserRetweets(string UserId, oAuthTwitter OAuth)
        {
            ...
            try
            {
                ...
                objTwitterMessage = new Domain.Myfashion.Domain.TwitterMessage();
        ...
        //        TwitterMessageRepository twtmsgrepo = new TwitterMessageRepository();
        //        // TwitterMessage twtmsg = new TwitterMessage();
        //        Domain.Myfashion.Domain.TwitterMessage ObjTwitterMessage = new Domain.Myfashion.Domain.TwitterMessage();
        //        foreach (var item in data)
        //        {
        ...
        //            TwitterEngagementRepository objTwitterEngagementRepository = new TwitterEngagementRepository();
        //            //TwitterMessageRepository objTwitterMessageRepository=new TwitterMessageRepository ();
        //            List<Domain.Myfashion.Domain.TwitterMessage> lsttwtmessage = objTwitterMessageRepository.getAllTwitterMessagesRetweet(userId, objTwitterAccount.TwitterUserId);
        //            foreach (var itemmsg in lsttwtmessage)
        //            {
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
        public void getUserTweets(oAuthTwitter OAuth, string TwitterScreenName, string TwitterUserId, Guid userId, Domain.Myfashion.Domain.TwitterAccount objTwitterAccount)
        {
            int I = 0;
            ...
            try
            {
                ...
                foreach (var item in data)
                {
                    ...
                    if (!twtmsgrepo.checkTwitterMessageExists(ObjTwitterMessage.ProfileId, ObjTwitterMessage.UserId, ObjTwitterMessage.MessageId))
                    {
                        I++;
                        twtmsgrepo.addTwitterMessage(ObjTwitterMessage);
                        logger.Error("getUserTweetsCount>>>"+I);
                    }
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
        public void gettwitterengagement(oAuthTwitter OAuth, Guid userId, Domain.Myfashion.Domain.TwitterAccount objTwitterAccount)
        {
            #region Add Twitter Engagement
            Tweet objTweet = new Tweet();
            TwitterEngagement objTwitterEngagement = new TwitterEngagement();
            ...
            //TwitterMessageRepository objTwitterMessageRepository=new TwitterMessageRepository ();
            List<Domain.Myfashion.Domain.TwitterMessage> lsttwtmessage = objTwitterMessageRepository.getAllTwitterMessagesRetweet(userId, objTwitterAccount.TwitterUserId);
            List<Domain.Myfashion.Domain.TwitterMessage> tsttwtwmessagetweet = objTwitterMessageRepository.getAllTwitterMessagestweet(userId, objTwitterAccount.TwitterUserId);
        ...
        [WebMethod]
        [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
        public void getTwitterStats(Domain.Myfashion.Domain.TwitterAccount twitterAccount)
        {
            //TwitterStatsRepository objTwtstats = new TwitterStatsRepository();
            TwitterStatsRepository objTwtstats = new TwitterStatsRepository();
            TwitterMessageRepository objTweetMsgRepo = new TwitterMessageRepository();
            //TwitterStats objStats = new TwitterStats();
            Domain.Myfashion.Domain.TwitterStats objStats = new Domain.Myfashion.Domain.TwitterStats();
            Random rNum = new Random();
        ...
        {
            List<Domain.Myfashion.Helper.TwitterRecentFollower> lstTwitterRecentFollower = new List<Domain.Myfashion.Helper.TwitterRecentFollower>();
            List<Domain.Myfashion.Domain.TwitterAccount> lstAccRepo = objTwitterAccountRepository.getAllTwitterAccountsOfUser(Guid.Parse(userid));
            oAuthTwitter oauth = null;
            GlobusTwitterLib.Twitter.Core.UserMethods.Users twtUsers = new GlobusTwitterLib.Twitter.Core.UserMethods.Users();
            foreach (Domain.Myfashion.Domain.TwitterAccount itemTwt in lstAccRepo)
            {
                    ...
                    if (item["ids"] != null)
                    {
                        foreach (var child in item["ids"])
                        {
                            if (resposecount < 2)
                            {
                                ...
                                {
                                    Domain.Myfashion.Helper.TwitterRecentFollower objTwitterRecentFollower = new Domain.Myfashion.Helper.TwitterRecentFollower();
                                    resposecount++;
        ...
        {
            Domain.Myfashion.Domain.TwitterAccount objTwitterAccount = objTwitterAccountRepository.GetUserInformation(Guid.Parse(userid), UserProfileId);
            oAuthTwitter OAuthTwt = new oAuthTwitter();
            OAuthTwt.AccessToken = objTwitterAccount.OAuthToken;
            OAuthTwt.AccessTokenSecret = objTwitterAccount.OAuthSecret;
            OAuthTwt.TwitterScreenName = objTwitterAccount.TwitterScreenName;
        ...
        //{
        //    List<Domain.Myfashion.Helper.TwitterProfileDetails> lstTwitterRecentFollower = new List<Domain.Myfashion.Helper.TwitterProfileDetails>();
        //    List<Domain.Myfashion.Domain.TwitterAccount> listallTwtaccount = objTwitterAccountRepository.getAllTwitterAccountsOfUser(Guid.Parse(userid));
        //    Domain.Myfashion.Helper.TwitterProfileDetails objTwtProfileDescription = new Domain.Myfashion.Helper.TwitterProfileDetails();
        //    oAuthTwitter OAuthTwt = new oAuthTwitter();
        //    foreach (Domain.Myfashion.Domain.TwitterAccount childnoe in listallTwtaccount)
        //    {
        ...
        public bool CheckTwitterTokenByUserId(oAuthTwitter objoAuthTwitter, string userid)
        {
            ...
            //oAuthTwitter oAuthTwt = new oAuthTwitter();
            GlobusTwitterLib.Twitter.Core.UserMethods.Users twtUser = new GlobusTwitterLib.Twitter.Core.UserMethods.Users();
            try
            {
                GlobusTwitterLib.Twitter.Core.UserMethods.Users userinfo = new GlobusTwitterLib.Twitter.Core.UserMethods.Users();
        ...
        {
            Domain.Myfashion.Domain.TwitterAccount objTwitterAccount = (Domain.Myfashion.Domain.TwitterAccount)(new JavaScriptSerializer().Deserialize(ObjTwitter, typeof(Domain.Myfashion.Domain.TwitterAccount)));
            try
            {
        ...
        {
            Domain.Myfashion.Domain.TwitterAccount objTwitterAccount = objTwitterAccountRepository.getUserInformation(profileid);
            oAuthTwitter OAuthTwt = new oAuthTwitter();
            OAuthTwt.AccessToken = objTwitterAccount.OAuthToken;
            OAuthTwt.AccessTokenSecret = objTwitterAccount.OAuthSecret;
            OAuthTwt.TwitterScreenName = objTwitterAccount.TwitterScreenName;
        ...
        public void getTwitterDirectMessageSent(string UserId, oAuthTwitter OAuth)
        {
            ...
            try
            {
                ...
                objTwitterDirectMessages = new Domain.Myfashion.Domain.TwitterDirectMessages();
