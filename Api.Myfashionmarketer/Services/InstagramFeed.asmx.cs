...
namespace Api.Myfashionmarketer.Models
{
    ...
    public class InstagramFeed : System.Web.Services.WebService
    {
        ...
        public string GetLinkedInFeeds(string UserId, string LinkedInId)
        {
            try
            {
                List<Domain.Myfashion.Domain.InstagramFeed> lstInstagramFeed = objInstagramFeedRepository.getAllInstagramFeedsOfUser(Guid.Parse(UserId), LinkedInId);
        ...
        public string GetFeedsOfProfileWithRange(string UserId, string LinkedInId, string noOfDataToSkip, string noOfDataFromTop)
        {
            try
            {
                List<Domain.Myfashion.Domain.InstagramFeed> lstInstagramFeed = objInstagramFeedRepository.getAllInstagramFeedsOfUser(Guid.Parse(UserId), LinkedInId, Convert.ToInt32(noOfDataToSkip), Convert.ToInt32(noOfDataFromTop));
                return new JavaScriptSerializer().Serialize(lstInstagramFeed);
            }
            catch (Exception ex)
            {
