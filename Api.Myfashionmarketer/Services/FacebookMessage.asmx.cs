    ...
    public class FacebookMessage : System.Web.Services.WebService
    {
        ...
        public string UserHomeWithLimit(string UserId, string FacebookId, string count)
        {
            try
            {
                ...
                List<Domain.Myfashion.Domain.FacebookMessage> lstfbmsgs = fbmsgrepo.getFacebookUserWallPost(userid, FacebookId, Convert.ToInt32(count));
        ...
        public string UserHome(string UserId, string FacebookId)
        {
            try
            {
                Guid userid = Guid.Parse(UserId);
                FacebookMessageRepository fbmsgrepo = new FacebookMessageRepository();
                List<Domain.Myfashion.Domain.FacebookMessage> lstfbmsgs = fbmsgrepo.getFacebookUserWallPost(userid, FacebookId);
                return new JavaScriptSerializer().Serialize(lstfbmsgs);
            }
            catch (Exception ex)
            {
        ...
        public string getAllFacebookMessagesOfUser(string UserId, string ProfileId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> objFacebookMessage = objFacebookMessageRepository.getAllFacebookMessagesOfUser(Guid.Parse(UserId), ProfileId);
        ...
        public string getAllSentMessages(string ProfileId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> objFacebookMessage = objFacebookMessageRepository.getAllSentMessages(ProfileId);
                return new JavaScriptSerializer().Serialize(objFacebookMessage);
            }
            catch (Exception ex)
            {
        ...
        public string GetAllWallpostsOfProfileAccordingtoGroup(string profileid, int count, string UserId)
        {
            try
            {
                ...
                List<Domain.Myfashion.Domain.FacebookMessage> objFacebookMessage = objFacebookMessageRepository.GetAllWallpostsOfProfileAccordingtoGroup(profileid, count, Guid.Parse(UserId));
                return new JavaScriptSerializer().Serialize(objFacebookMessage);
            }
            catch (Exception ex)
            {
        ...
        {
            //List<Domain.Myfashion.Domain.FacebookMessage> lstmessage = new List<Domain.Myfashion.Domain.FacebookMessage>();
            int lstmessage = 0;
            try
            {
        ...
        public string getAllFacebookMessagesOfUserByProfileId(string ProfileId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> objFacebookMessage = objFacebookMessageRepository.getAllMessageOfProfile(ProfileId);
                return new JavaScriptSerializer().Serialize(objFacebookMessage);
            }
            catch (Exception ex)
            {
        ...
        //    try
        //    {
        //        List<Domain.Myfashion.Domain.FacebookMessage> objFacebookMessage = objFacebookMessageRepository.getAllMessageOfProfileWithRange(ProfileId, noOfDataToSkip);
        //        return new JavaScriptSerializer().Serialize(objFacebookMessage);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        public string getAllFacebookTagOfUsers(string UserId, string ProfileId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> lstFacebookTag = objFacebookMessageRepository.getAllFacebookTagOfUsers(Guid.Parse(UserId), ProfileId);
        ...
        public string getAllFacebookstatusOfUsers(string UserId, string ProfileId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> lstFacebookstatus = objFacebookMessageRepository.getAllFacebookstatusOfUsers(Guid.Parse(UserId), ProfileId);
        ...
        public string getAllFacebookUserFeedOfUsers(string UserId, string ProfileId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> lstFacebookUserFeed = objFacebookMessageRepository.getAllFacebookUserFeedOfUsers(Guid.Parse(UserId), ProfileId);
        ...
        {
            Domain.Myfashion.Domain.FacebookMessage objFacebookMessage = objFacebookMessageRepository.GetMessageDetailByMessageid(MessageId);
            return new JavaScriptSerializer().Serialize(objFacebookMessage);
        ...
        {
            Domain.Myfashion.Domain.FacebookMessage objFacebookMessage = objFacebookMessageRepository.GetFacebookMessageByMessageId(Guid.Parse(userid), msgid);
            return new JavaScriptSerializer().Serialize(objFacebookMessage);
        }
        [WebMethod]
        [ScriptMethod(UseHttpGet = false, ResponseFormat = ResponseFormat.Json)]
        ...
        {
            List<Domain.Myfashion.Domain.FacebookMessage> lstFacebookMessage = objFacebookMessageRepository.getAllMessageDetail(profileid);
        ...
        //{
        //    List<Domain.Myfashion.Domain.FacebookMessage> lstFacebookMessage = objFacebookMessageRepository.getAllMessageDetail(profileid, noOfDataToSkip);
        //    return new JavaScriptSerializer().Serialize(lstFacebookMessage);
        //}
        ...
        public string GetAllWallpostsOfProfileAccordingtoGroupByUserIdAndProfileId1WithRange(string UserId, string keyword, string ProfileId, string count)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> objFacebookMessage = objFacebookMessageRepository.GetAllWallpostsOfProfileAccordingtoGroupByUserIdAndProfileId1WithRange(UserId, keyword, ProfileId, count);
                return new JavaScriptSerializer().Serialize(objFacebookMessage);
            }
            catch (Exception ex)
            {
        ...
        public string getAllFacebookUserFeedOfUsersByUserIdAndProfileId1WithRange(string UserId, string keyword, string ProfileId, string count)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> lstFacebookUserFeed = objFacebookMessageRepository.getAllFacebookUserFeedOfUsersByUserIdAndProfileId1WithRange(UserId, keyword, ProfileId, count);
                return new JavaScriptSerializer().Serialize(lstFacebookUserFeed);
            }
            catch (Exception ex)
            {
        ...
        public string getAllFacebookMessagesOfUserByProfileIdWithRange(string ProfileId, string noOfDataToSkip, string UseId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookMessage> objFacebookMessage = objFacebookMessageRepository.getAllMessageOfProfileWithRange(ProfileId, noOfDataToSkip, Guid.Parse(UseId));
                return new JavaScriptSerializer().Serialize(objFacebookMessage);
            }
            catch (Exception ex)
            {
        ...
        {
            List<Domain.Myfashion.Domain.FacebookMessage> lstFacebookMessage = objFacebookMessageRepository.getAllMessageDetail(profileid, noOfDataToSkip, Guid.Parse(UserId));
            return new JavaScriptSerializer().Serialize(lstFacebookMessage);
        }
