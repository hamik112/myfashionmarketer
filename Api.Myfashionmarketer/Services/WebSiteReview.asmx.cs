...
namespace Api.Myfashionmarketer.Services
{
    ...
    {
        ILog logger = LogManager.GetLogger(typeof(WebSiteReview));
        ...
        public string WebSiteData(string url)
        {
            ...
            if (!objWebSiteReviewDataRpository.IswebsitenameExist(url))
            {
                ...
                GlobusHttpHelper _GlobusHttpHelper = new GlobusHttpHelper();
                 ...
                 //if (pagesource.Contains("window.NREUM") || pagesource.Contains("NREUM.info ="))
                 //{
                 //    pagesource = _GlobusHttpHelper.getHtmlfromUrlProxy(new Uri(Url), "188.240.133.117", 3128, "", "");
                 //}
                 //if (pagesource.Contains("window.NREUM") || pagesource.Contains("NREUM.info =") ||string.IsNullOrEmpty(pagesource))
                 //{
                 //    pagesource = _GlobusHttpHelper.getHtmlfromUrlProxy(new Uri(Url), "95.85.29.99", 1603, "mongoose", "Fjh30fi");
                 //}
                 //if (pagesource.Contains("window.NREUM") || pagesource.Contains("NREUM.info =") || string.IsNullOrEmpty(pagesource))
                 //{
                 //    pagesource = _GlobusHttpHelper.getHtmlfromUrlProxy(new Uri(Url), "195.154.161.103", 1601, "mongoose", "Fjh30fi");
                 //}
        ...
        public string GetAllWebSite()
        {
            try
            {
                WebSiteReviewDataRpository _objWebSiteReviewDataRpository = new WebSiteReviewDataRpository();
                List<websitereviewdata> lstwebsitedata = _objWebSiteReviewDataRpository.GetAllwebsite();
                return new JavaScriptSerializer().Serialize(lstwebsitedata);
            }
            catch (Exception ex)
            {
