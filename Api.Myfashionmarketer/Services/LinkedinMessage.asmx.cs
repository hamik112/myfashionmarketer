...
namespace Api.Myfashionmarketer.Models
{
    ...
    public class LinkedinMessage : System.Web.Services.WebService
    {
        ...
        public string GetLinkedInMessages(string UserId, string LinkedInId, int count)
        {
            try
            {
                List<Domain.Myfashion.Domain.LinkedInMessage> lstlinkedinmessages = objLinkedInMessageRepository.getLinkedInMessageDetail(LinkedInId, count.ToString(), Guid.Parse(UserId));
