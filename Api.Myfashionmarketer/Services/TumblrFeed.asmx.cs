...
namespace Api.Myfashionmarketer.Models
{
    ...
    public class TumblrFeed : System.Web.Services.WebService
    {
        ...
        public string GetAllTumblrFeedOfUsers(string UserId, string ProfileId)
        {
            try
            {
                List<Domain.Myfashion.Domain.TumblrFeed> lstTumblrFeed = objTumblrFeedRepository.GetFeedsOfProfile(ProfileId, Guid.Parse(UserId));
        ...
        public string GetAllTumblrFeedOfUsersWithRange(string UserId, string ProfileId, string noOfDataToSkip)
        {
            try
            {
                List<Domain.Myfashion.Domain.TumblrFeed> lstTumblrFeed = objTumblrFeedRepository.GetFeedsOfProfileWithRange(ProfileId, Guid.Parse(UserId), Convert.ToInt32(noOfDataToSkip));
                return new JavaScriptSerializer().Serialize(lstTumblrFeed);
            }
            catch (Exception ex)
            {
