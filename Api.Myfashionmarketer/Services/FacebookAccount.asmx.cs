...
namespace Api.Myfashionmarketer.Models
{
    ...
    public class FacebookAccount : System.Web.Services.WebService
    {
        ...
        Domain.Myfashion.Domain.FacebookAccount objFacebook;
        [WebMethod]
        ...
        public string GetFacebookAccountDetailsById(string UserId)
        {
            try
            {
                Domain.Myfashion.Domain.FacebookAccount objFacebookAccount = objFacebookAccountRepository.getFacebookAccountDetailsById(UserId);
                return new JavaScriptSerializer().Serialize(objFacebookAccount);
            }
            catch (Exception ex)
            {
        ...
        public string GetFacebookAccountByUserId(string UserId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookAccount> lstFacebookAccount = objFacebookAccountRepository.GetAllFacebookAccountByUserId(Guid.Parse(UserId));
        ...
        public string getAllFacebookAccountsOfUser(string UserId)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookAccount> objFacebookAccount = objFacebookAccountRepository.getAllFacebookAccountsOfUser(Guid.Parse(UserId));
                return new JavaScriptSerializer().Serialize(objFacebookAccount);
            }
            catch (Exception ex)
            {
        ...
        public string GetAllFacebookPageByUserIdAndGroupId(string userid, string groupid)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookAccount> lstFacebookAccount = new List<Domain.Myfashion.Domain.FacebookAccount>();
                Domain.Myfashion.Domain.Team objTeam = objTeamRepository.GetTeamByGroupId(Guid.Parse(groupid));
                List<Domain.Myfashion.Domain.TeamMemberProfile> lstTeamMemberProfile = objTeamMemberProfileRepository.GetTeamMemberProfileByTeamIdAndProfileType(objTeam.Id, "facebook_page");
                foreach (var item in lstTeamMemberProfile)
                {
                    try
                    {
        ...
        public string GetAllFacebookAccountDetails(string profileid,string userid)
        {
            try
            {
                List<Domain.Myfashion.Domain.FacebookAccount> lstfbprofileDetails = objFacebookAccountRepository.getAllAccountDetail(profileid,Guid.Parse(userid));
