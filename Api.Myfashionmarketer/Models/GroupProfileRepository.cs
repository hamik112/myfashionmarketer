...
namespace Api.Myfashionmarketer.Services
{
    public class GroupProfileRepository
    {
        ...
        public List<Domain.Myfashion.Domain.GroupProfile> getAllGroupProfiles(Guid Userid, Guid groupid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    List<Domain.Myfashion.Domain.GroupProfile> alstFBAccounts = session.CreateQuery("from GroupProfile where GroupOwnerId = :userid and GroupId =:groupid")
                    .SetParameter("userid", Userid)
                    .SetParameter("groupid", groupid)
                    .List<Domain.Myfashion.Domain.GroupProfile>()
                    .ToList<Domain.Myfashion.Domain.GroupProfile>();
        ...
        /// <param name="groupid">Id of group (Guid)</param>
        /// <returns>List of group profiles.(List<GroupProfile>)</returns>
        public List<Domain.Myfashion.Domain.GroupProfile> GetAllGroupProfiles(Guid groupid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                {
                    //Proceed action, to find details of group by user id and group id.
                    List<Domain.Myfashion.Domain.GroupProfile> alstFBAccounts = session.CreateQuery("from GroupProfile where GroupId =:groupid")
                    .SetParameter("groupid", groupid)
                    .List<Domain.Myfashion.Domain.GroupProfile>()
                    .ToList<Domain.Myfashion.Domain.GroupProfile>();
