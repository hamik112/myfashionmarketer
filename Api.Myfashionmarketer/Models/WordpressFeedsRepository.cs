...
namespace Api.Myfashionmarketer.Models
{
    public class WordpressFeedsRepository
    {
        ...
        public List<Domain.Myfashion.Domain.WordpressFeeds> GetAllSiteFeedBySiteId(Guid UserId, string SiteId)
        {
              using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //Begin session trasaction and opens up.
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<Domain.Myfashion.Domain.WordpressFeeds> query = session.CreateQuery("from WordpressFeeds where UserId = :userid and SiteId = : SiteId")
                        .SetParameter("userid", UserId)
                        .SetParameter("SiteId", SiteId)
                        .List<Domain.Myfashion.Domain.WordpressFeeds>().ToList<Domain.Myfashion.Domain.WordpressFeeds>();
                        return query;
                    }
                    catch(Exception ex)
                    {
                        return new List<Domain.Myfashion.Domain.WordpressFeeds>();
                    }
                }
              }
