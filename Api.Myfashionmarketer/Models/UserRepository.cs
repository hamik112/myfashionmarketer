...
namespace Api.Myfashionmarketer.Models
{
    public class UserRepository
    {
        ...
        public Domain.Myfashion.Domain.User GetUserInfoByCode(string code)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        return (Domain.Myfashion.Domain.User)query.UniqueResult();
                    }
                    catch (Exception ex)
                    {
                        return null;
        ...
        public List<Domain.Myfashion.Domain.User> login(string email, string pass)
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    try
                    {
                        List<Domain.Myfashion.Domain.User> _login = session.Query<Domain.Myfashion.Domain.User>().Where(u => u.EmailId == email && u.Password == pass).ToList();
                        return _login;
                    }
                    catch (Exception ex)
                    {
        ...
        public List<User> Employees(Guid c_id)
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<User> result = session.Query<User>().Where(e => e.Company_id == c_id).ToList();
                        return result;
                    }
                    catch (Exception ex)
                    {
        ...
        public User contact(Guid Id)
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        NHibernate.IQuery query = session.CreateQuery("from User u where u.Id = : Id");
