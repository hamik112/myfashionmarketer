...
namespace Api.Myfashionmarketer.Models
{
    public class InstagramFeedRepository : IInstagramFeed
    {
        ...
        public void addInstagramFeed(Domain.Myfashion.Domain.InstagramFeed insfeed)
        {
        ...
        public int deleteInstagramFeed(Domain.Myfashion.Domain.InstagramFeed fbfeed)
        {
            throw new NotImplementedException();
        }

        public int updateInstagramFeed(Domain.Myfashion.Domain.InstagramFeed fbfeed)
        {
            throw new NotImplementedException();
        }
        ...
        public List<Domain.Myfashion.Domain.InstagramFeed> getAllInstagramFeedsOfUser(Guid UserId, string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.InstagramFeed> alst = session.CreateQuery("from InstagramFeed where UserId = :userid and InstagramId = :profileId")
                      .SetParameter("userid", UserId)
                       .SetParameter("profileId", profileid)
                       .List<Domain.Myfashion.Domain.InstagramFeed>()
                       .ToList<Domain.Myfashion.Domain.InstagramFeed>();
        ...
        public List<Domain.Myfashion.Domain.InstagramFeed> getAllInstagramFeedsOfUser(Guid UserId, string profileid, int noOfDataToSkip, int noOfDataFromTop)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.InstagramFeed> alst = session.CreateQuery("from InstagramFeed where UserId = :userid and InstagramId = :profileId Order By FeedDate DESC")
                        .SetParameter("userid", UserId)
                        .SetParameter("profileId", profileid)
        ...
        public List<Domain.Myfashion.Domain.InstagramFeed> GetFeedsOfProfileWithRange(string profileid, Guid id, int noOfDataToSkip)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        //List<Domain.Myfashion.Domain.TumblrFeed> lstmsg = session.CreateQuery("from TumblrFeed where  ProfileId = :profid and UserId=:userid ")
                        //.SetParameter("profid", profileid)
                        //.SetParameter("userid", id)
                        //.List<Domain.Myfashion.Domain.TumblrFeed>()
                        //.ToList<Domain.Myfashion.Domain.TumblrFeed>();

                        List<Domain.Myfashion.Domain.InstagramFeed> lstmsg = session.Query<Domain.Myfashion.Domain.InstagramFeed>().Where(u => u.UserId == id && u.InstagramId.Equals(profileid)).OrderByDescending(x => x.EntryDate).Skip(Convert.ToInt32(noOfDataToSkip)).Take(15).ToList<Domain.Myfashion.Domain.InstagramFeed>();
