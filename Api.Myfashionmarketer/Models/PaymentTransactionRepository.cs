...
namespace SocioBoard.Model
{
    public class PaymentTransactionRepository
    {
        ...
        public List<Domain.Myfashion.Domain.PaymentTransaction> GetPaymentDataByUserId(Guid UserId)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<Domain.Myfashion.Domain.PaymentTransaction> lst = session.CreateQuery("from PaymentTransaction where UserId=: userid ORDER BY PaymentDate DESC")
                                       .SetParameter("userid", UserId)
                                       .List<Domain.Myfashion.Domain.PaymentTransaction>().ToList<Domain.Myfashion.Domain.PaymentTransaction>();
                        return lst;
                    }
                    catch (Exception ex)
                    {
