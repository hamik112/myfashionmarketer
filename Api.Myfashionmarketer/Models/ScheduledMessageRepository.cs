...
namespace Api.Myfashionmarketer.Models
{
    public class ScheduledMessageRepository : IScheduledMessageRepository
    {
        ...
        public void addNewMessage(Domain.Myfashion.Domain.ScheduledMessage schmesg)
        {
        ...
        public void deleteMessage(Domain.Myfashion.Domain.ScheduledMessage schmesg)
        {
            //Creates a database connection and opens up a session.
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessageDetailsbyDate(Guid userid, string profileid, int days)
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        str += ")";
                        List<Domain.Myfashion.Domain.ScheduledMessage> alst = session.CreateQuery(str).SetParameter("userid", userid)
                       .List<Domain.Myfashion.Domain.ScheduledMessage>().Where(d=>d.ScheduleTime.Date>=AssinDate.Date)
                       .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
                        return alst;
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessageDetailsforADay(Guid userid, string profileid, int days)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.ScheduledMessage> alst = session.CreateQuery(str).SetParameter("userid", userid)
                       .List<Domain.Myfashion.Domain.ScheduledMessage>()
                       .ToList<Domain.Myfashion.Domain.ScheduledMessage>().Where(a => a.ScheduleTime.Date == DateTime.Now.AddDays(-days).Date).ToList<Domain.Myfashion.Domain.ScheduledMessage>();
                        return alst;
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessageDetailsByDays(Guid userid, string profileid, int days)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessageDetailsByMonth(Guid userid, string profileid, int month)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.ScheduledMessage> alst = session.CreateQuery(str).SetParameter("userid", userid)
                       .List<Domain.Myfashion.Domain.ScheduledMessage>()
                       .ToList<Domain.Myfashion.Domain.ScheduledMessage>().Where(a => a.ScheduleTime.Month == DateTime.Now.AddMonths(-month).Month).ToList<Domain.Myfashion.Domain.ScheduledMessage>();
                        return alst;
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessageDetailsForCustomrange(Guid userid, string profileid, DateTime sdate, DateTime ldate)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.ScheduledMessage> alst = session.CreateQuery(str).SetParameter("userid", userid)
                       .List<Domain.Myfashion.Domain.ScheduledMessage>()
                       .ToList<Domain.Myfashion.Domain.ScheduledMessage>().Where(a => a.ScheduleTime.Date >= sdate && a.ScheduleTime.Date <= ldate).ToList<Domain.Myfashion.Domain.ScheduledMessage>();
                        return alst;
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllMessagesOfUser(Guid userid, string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschmsg = session.CreateQuery("from ScheduledMessage where UserId = :userid and ScheduleTime > :schtime and ProfileId =:profileid and Status = 0")
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllMessagesOfUser(string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //proceed action, to get all messages by user id and profileid.
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschmsg = session.CreateQuery("from ScheduledMessage where  ScheduleTime > :schtime and ProfileId =:profileid and Status = 0 order by ScheduleTime desc")
                       .SetParameter("schtime", DateTime.Now)
                       .SetParameter("profileid", profileid)
                       .List<Domain.Myfashion.Domain.ScheduledMessage>()
                       .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
        ...
        public Domain.Myfashion.Domain.ScheduledMessage GetScheduledMessageDetails(Guid id)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //proceed action, to get all messages by user id and profileid.
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschmsg = session.CreateQuery("from ScheduledMessage where Id=:id ")
                       .SetParameter("id", id)
                       .List<Domain.Myfashion.Domain.ScheduledMessage>()
                       .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
                        return lstschmsg[0];
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> GetUnsentSchdeuledMessageByProfileType(string profiletype)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //proceed action, to get all messages by user id and profileid.
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschmsg = session.CreateQuery("from ScheduledMessage where  ScheduleTime <= :schtime and ProfileType =:profiletype and Status = 0 order by ScheduleTime desc")
                    ...
                    {
                        Console.WriteLine(ex.StackTrace);
                        return new List<Domain.Myfashion.Domain.ScheduledMessage>();
                    }
                }//End Transaction
            }//End Session
        }
        ...
        public IEnumerable<Domain.Myfashion.Domain.ScheduledMessage> getAllMessages(DateTime date)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                try
                {
                    ...
                    return session.CreateCriteria(typeof(Domain.Myfashion.Domain.ScheduledMessage)).List<Domain.Myfashion.Domain.ScheduledMessage>().Where(x => x.Status == false && x.ScheduleTime <= date);
        ...
        /// </summary>
        /// <returns>Return object of ScheduledMessage Class with  value of each member in the form of IEnumerable list.(IEnumerable<ScheduledMessage>)</returns>
        public IEnumerable<Domain.Myfashion.Domain.ScheduledMessage> getAllMessage()
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                {
                    //Proceed action, to get all records of scheduled message.
                    return session.CreateCriteria(typeof(Domain.Myfashion.Domain.ScheduledMessage)).List<Domain.Myfashion.Domain.ScheduledMessage>().Where(x => x.Status == false && x.ScheduleTime <= DateTime.Now);
        ...
        /// <returns>Return object of ScheduledMessage Class with  value of each member in the form of  list.(List<ScheduledMessage>)</returns>
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllmessages(DateTime schtime)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschtime = session.CreateQuery("from ScheduledMessage where ScheduleTime <= :shctime and Status = false")
        ...
        /// <param name="UserId">User id (Guid)</param>
        /// <returns>Return object of ScheduledMessage Class with  value of each member in the form of  list.(List<ScheduledMessage>)</returns>
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllMessagesOfUser(Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action, to get all records of scheduled message.
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschtime = session.CreateQuery("from ScheduledMessage where UserId =:userid order by ScheduleTime desc")
                        .SetParameter("userid", UserId)
                        .List<Domain.Myfashion.Domain.ScheduledMessage>()
                        .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllMessagesDetail(string profileid, Guid userid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        str = str.Substring(0, str.Length - 1);
                        str += ") order by ScheduleTime desc ";
                        List<Domain.Myfashion.Domain.ScheduledMessage> alst = session.CreateQuery(str).SetParameter("userid", userid).SetFirstResult(0).SetMaxResults(20)
                       .List<Domain.Myfashion.Domain.ScheduledMessage>()
                       .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
                        return alst;
        ...
        //public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessageDetails(string profileid)
        //{
        //    //Creates a database connection and opens up a session
        //    using (NHibernate.ISession session = SessionFactory.GetNewSession())
        //    {
        ...
        //                str = str.Substring(0, str.Length - 1);
        //                str += ")";
        //                List<Domain.Myfashion.Domain.ScheduledMessage> alst = session.CreateQuery(str)
        //               .List<Domain.Myfashion.Domain.ScheduledMessage>()
        //               .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
        //                return alst;
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessageDetails(string profileid, Guid userid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
        ...
        /// <param name="UserId">User id.(Guid)</param>
        /// <returns>Return object of ScheduledMessage Class with  value of each member in the form of  list.(List<ScheduledMessage>)</returns>
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllIUnSentMessagesOfUser(Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschtime = session.CreateQuery("from ScheduledMessage where Status = 0 and UserId =:userid")
                          .SetParameter("userid", UserId)
                          .List<Domain.Myfashion.Domain.ScheduledMessage>()
                          .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessagesOfUser(Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action,to get all unread messages.
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschtime = session.CreateQuery("from ScheduledMessage where Status = 1 and UserId =:userid order by ClientTime desc")
                          .SetParameter("userid", UserId)
                          .List<Domain.Myfashion.Domain.ScheduledMessage>()
                          .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> GetAllUnSentMessagesAccordingToGroup(Guid UserId, string profileid, string network)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action,to get all unread messages.
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschtime = session.CreateQuery("from ScheduledMessage where Status = 0 and UserId =:userid and ProfileType=:profiletype and ProfileId=:profileid order by ClientTime desc")
                          .SetParameter("userid", UserId)
        ...
        public List<Domain.Myfashion.Domain.ScheduledMessage> getAllSentMessagesOfUser(string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action,to get all unread messages.
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschtime = session.CreateQuery("from ScheduledMessage where Status = 1 and ProfileId =:profileid order by ClientTime desc")
                          .SetParameter("profileid", profileid)
                          .List<Domain.Myfashion.Domain.ScheduledMessage>()
                          .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
        ...
        /// <param name="UserId">User id.(Guid)</param>
        /// <returns>Return object of ScheduledMessage Class with  value of each member in the form of  list.(List<ScheduledMessage>)</returns>
        public List<Domain.Myfashion.Domain.ScheduledMessage> getWooQueueMessage(Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.ScheduledMessage> lstschtime = session.CreateQuery("from ScheduledMessage where Status = 0 and UserId =:userid and ScheduleTime >= :schtime")
                        .SetParameter("userid", UserId)
                        .SetParameter("schtime", DateTime.Now)
                        .List<Domain.Myfashion.Domain.ScheduledMessage>()
                        .ToList<Domain.Myfashion.Domain.ScheduledMessage>();
        ...
        public Domain.Myfashion.Domain.ScheduledMessage getScheduleMessageByMessageId(Guid Id)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        Domain.Myfashion.Domain.ScheduledMessage lstschtime = query.UniqueResult<Domain.Myfashion.Domain.ScheduledMessage>();
                        return lstschtime;
                    }
                    catch (Exception ex)
                    {
        ...
        /// </summary>
        /// <returns>Return object of ScheduledMessage Class with  value of each member in the form of  list.(List<ScheduledMessage>)</returns>
        public List<Domain.Myfashion.Helper.ScheduledTracker> GetAllScheduledDetails()
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        var results = session.QueryOver<Domain.Myfashion.Domain.ScheduledMessage>().Where(U => U.UserId == UserId).AndRestrictionOn(m => m.ProfileId).IsIn(arrsrt).Select(Projections.RowCount()).FutureValue<int>().Value;
