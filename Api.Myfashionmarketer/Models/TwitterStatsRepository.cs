...
namespace Api.Myfashionmarketer.Models
{
    public class TwitterStatsRepository : ITwitterStatsRepository
    {
        ...
        public void addTwitterStats(Domain.Myfashion.Domain.TwitterStats TwtStats)
        {
        ...
        public int updateTwitterStats(Domain.Myfashion.Domain.TwitterStats TwtStats)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        foreach (Domain.Myfashion.Domain.TwitterStats item in query.Enumerable())
                        {
                            alstTwtStats.Add(item);
                        }
                        return alstTwtStats;
