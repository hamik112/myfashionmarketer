...
namespace Api.Myfashionmarketer.Models
{
    public class LinkedInFeedRepository : ILinkedInFeedRepository
    {
        ...
        public void addLinkedInFeed(Domain.Myfashion.Domain.LinkedInFeed lifeed)
        {
        ...
        public int deleteLinkedInFeed(Domain.Myfashion.Domain.LinkedInFeed lifeed)
        {
            throw new NotImplementedException();
        }

        public int updateLinkedInFeed(Domain.Myfashion.Domain.LinkedInFeed lifeed)
        {
            throw new NotImplementedException();
        }
        ...
        public List<Domain.Myfashion.Domain.LinkedInFeed> getAllLinkedInFeedsOfUser(Guid UserId, string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.LinkedInFeed> alst = session.CreateQuery("from LinkedInFeed where UserId = :userid and ProfileId = :profileId")
                        .SetParameter("userid", UserId)
        ...
        public List<Domain.Myfashion.Domain.LinkedInFeed> getAllLinkedInFeedsOfUserOfSBUserWithRangeAndProfileId(Guid UserId, string profileid, string noOfDataToSkip, string noOfResultsFromTop)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action, to get linkedin feeds
                        List<Domain.Myfashion.Domain.LinkedInFeed> alst = session.Query<Domain.Myfashion.Domain.LinkedInFeed>().Where(x => x.ProfileId.Equals(profileid) && x.UserId.Equals(UserId)).OrderByDescending(x => x.FeedsDate).Skip(Convert.ToInt32(noOfDataToSkip)).Take(Convert.ToInt32(noOfResultsFromTop))
                        .ToList<Domain.Myfashion.Domain.LinkedInFeed>();
                        return alst;
        ...
        public List<Domain.Myfashion.Domain.LinkedInFeed> getAllLinkedInFeedsOfUserOfSBUserWithRangeByProfileId(string profileid, string noOfDataToSkip, string noOfResultsFromTop)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action, to get linkedin feeds
                        List<Domain.Myfashion.Domain.LinkedInFeed> alst = session.Query<Domain.Myfashion.Domain.LinkedInFeed>().Where(x => x.ProfileId.Equals(profileid)).OrderByDescending(x => x.FeedsDate).Skip(Convert.ToInt32(noOfDataToSkip)).Take(Convert.ToInt32(noOfResultsFromTop))
                        .ToList<Domain.Myfashion.Domain.LinkedInFeed>();
                        return alst;
                    }
                    catch (Exception ex)
        ...
        public List<Domain.Myfashion.Domain.LinkedInFeed> getAllLinkedInUserFeeds(string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.LinkedInFeed> alst = session.CreateQuery("from LinkedInFeed where  ProfileId = :profileId ORDER BY FeedsDate DESC")
                        .SetParameter("profileId", profileid)
                        .List<Domain.Myfashion.Domain.LinkedInFeed>()
                        .ToList<Domain.Myfashion.Domain.LinkedInFeed>();
        ...
        /// <param name="profileid">Profile id of linkedin (String)</param>
        /// <returns>Return object of LinkedInFeed Class with value of each member in the form of list.(List<LinkedInFeed>)</returns>
        public List<Domain.Myfashion.Domain.LinkedInFeed> getAllLinkedInFeedsOfUser(Guid UserId, string profileid, int count)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
        ...
        /// <returns>Return object of LinkedInFeed Class with value of each member in the form of list.(List<LinkedInFeed>)</returns>
        public List<Domain.Myfashion.Domain.LinkedInFeed> getAllLinkedInFeedsOfProfile(string ProfileId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.LinkedInFeed> alst = session.CreateQuery("from LinkedInFeed where ProfileId = :ProfileId ORDER BY EntryDate DESC")
                        .SetParameter("ProfileId", ProfileId)
                        .List<Domain.Myfashion.Domain.LinkedInFeed>()
                        .ToList<Domain.Myfashion.Domain.LinkedInFeed>();
        ...
        public List<Domain.Myfashion.Domain.LinkedInFeed> getAllLinkedInFeedsOfProfileWithId(string ProfileId, string Id)
        {
            List<Domain.Myfashion.Domain.LinkedInFeed> objlist = new List<Domain.Myfashion.Domain.LinkedInFeed>();
