...
namespace Api.Myfashionmarketer.Models
{
    public class TaskCommentRepository : ITaskCommentRepository
    {
        ...
        public void addTaskComment(Domain.Myfashion.Domain.TaskComment taskcomment)
        {
        ...
        public List<Domain.Myfashion.Domain.TaskComment> getAllTasksCommentOfUser(Guid TaskId, Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    List<Domain.Myfashion.Domain.TaskComment> alst = session.CreateQuery("from TaskComment where  TaskId=:taskid and UserId=:userId")
                      .SetParameter("taskid", TaskId)
                      .SetParameter("userId", UserId)
                      .List<Domain.Myfashion.Domain.TaskComment>()
                      .ToList<Domain.Myfashion.Domain.TaskComment>();
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        for (int i = 0; i < 12; i++)
                        {
                            ...
                            List<Domain.Myfashion.Domain.TaskComment> lstFacebookStats = session.CreateQuery("from TaskComment where CommentDate Like :year and UserId= :userId ")
                                .SetParameter("userId", Userid)
                                .SetParameter("year", "%" + year + "%", TypeFactory.GetAnsiStringType(15)).List<Domain.Myfashion.Domain.TaskComment>().ToList<Domain.Myfashion.Domain.TaskComment>();
                            //add in list
