...
namespace Api.Myfashionmarketer.Models
{
    ...
    {
        public static void Add(Domain.Myfashion.Domain.YoutubeChannel YoutubeChannel)
        {
        ...
        public Domain.Myfashion.Domain.YoutubeChannel getYoutubeChannelDetailsById(string youtubeId)
        {

            Domain.Myfashion.Domain.YoutubeChannel result = new Domain.Myfashion.Domain.YoutubeChannel();
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    List<Domain.Myfashion.Domain.YoutubeChannel> objlstfb = session.CreateQuery("from YoutubeChannel where Googleplususerid = :youtubeId ")
                            .SetParameter("youtubeId", youtubeId)
                       .List<Domain.Myfashion.Domain.YoutubeChannel>().ToList<Domain.Myfashion.Domain.YoutubeChannel>();
                    if (objlstfb.Count > 0)
                    {
                        result = objlstfb[0];
                    }
        ...
        public Domain.Myfashion.Domain.YoutubeChannel getYoutubeChannelDetailsById(string youtubeId, Guid userid)
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    // proceed action, to get all Facebook Account of User by UserId(Guid) and FbUserId(string).
                    List<Domain.Myfashion.Domain.YoutubeChannel> objlstfb = session.CreateQuery("from YoutubeChannel where Googleplususerid = :youtubeId and UserId=:userid ")
                            .SetParameter("youtubeId", youtubeId)
