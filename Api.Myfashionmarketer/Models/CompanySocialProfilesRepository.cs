...
namespace Api.Socioboard.Model
{
    public class CompanySocialProfilesRepository
    {
        ...
        public bool updatecompanyProfileName(Domain.Myfashion.Domain.CompanyProfiles companyProfiles)
        {
            bool isUpdate = false;
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
        ...
        public Domain.Myfashion.Domain.CompanyProfiles getCompanyProfiles(Guid  Id)
        {
            Domain.Myfashion.Domain.CompanyProfiles companyProfile = null;
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //After Session creation, start Transaction.
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        Domain.Myfashion.Domain.CompanyProfiles result = (Domain.Myfashion.Domain.CompanyProfiles)query.UniqueResult();
        ...
        public Domain.Myfashion.Domain.CompanyProfiles SearchCompanyName(string Keywords)
        {
            Domain.Myfashion.Domain.CompanyProfiles companyProfile = null;
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    List<Domain.Myfashion.Domain.CompanyProfiles> result = session.CreateQuery("from CompanyProfiles where Companyname=:CompanyName").SetParameter("CompanyName", Keywords).List<Domain.Myfashion.Domain.CompanyProfiles>().ToList<Domain.Myfashion.Domain.CompanyProfiles>();
                    try
                    {
