...
namespace Api.Myfashionmarketer.Models
{
    ...
    {
        public void AddInboxMessages(Domain.Myfashion.Domain.InboxMessages _InboxMessages)
        {
        ...
        public Domain.Myfashion.Domain.InboxMessages getInboxMessageByMessageId(Guid UserId, Guid MessageId) 
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        Domain.Myfashion.Domain.InboxMessages result = (Domain.Myfashion.Domain.InboxMessages)results.UniqueResult();
                        return result;
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(ex.StackTrace);
                        return new Domain.Myfashion.Domain.InboxMessages();
                    }
                }//End using transaction
            }//End using session
