...
namespace Api.Myfashionmarketer.Models
{
    public class TumblrFeedRepository
    {
        ...
        public List<Domain.Myfashion.Domain.TumblrFeed> getFeedOfProfile(string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.TumblrFeed> lstmsg = session.CreateQuery("from TumblrFeed where  ProfileId = :profid group by blogId")
        ...
        public List<Domain.Myfashion.Domain.TumblrFeed> GetFeedsOfProfile(string profileid, Guid id)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
        ...
        public List<Domain.Myfashion.Domain.TumblrFeed> GetFeedsOfProfileWithRange(string profileid, Guid id, int noOfDataToSkip)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.TumblrFeed> lstmsg = session.Query<Domain.Myfashion.Domain.TumblrFeed>().Where(u => u.UserId == id && u.ProfileId.Equals(profileid)).OrderByDescending(x => x.date).Skip(Convert.ToInt32(noOfDataToSkip)).Take(6).ToList<Domain.Myfashion.Domain.TumblrFeed>();
