...
namespace Api.Myfashionmarketer.Models
{
    public class DraftsRepository : IDraftsRepository
    {
        ...
        public List<Domain.Myfashion.Domain.Drafts> getAllDrafts(Guid groupid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.Drafts> lst = session.CreateQuery("From Drafts where GroupId = :groupid")
                                           .SetParameter("groupid", groupid)
                                           .List<Domain.Myfashion.Domain.Drafts>()
                                           .ToList<Domain.Myfashion.Domain.Drafts>();
                        #region oldcode
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.Drafts> lst = new List<Domain.Myfashion.Domain.Drafts>();
                        foreach (Domain.Myfashion.Domain.Drafts item in query.Enumerable<Domain.Myfashion.Domain.Drafts>())
                        {
                            lst.Add(item);
                        }
                        if (lst.Count == 0)
        ...
        /// <param name="UserId">Pass the Guid of user.</param>
        /// <returns>List of Draft messages.</returns>
        public List<Domain.Myfashion.Domain.Drafts> GetDraftMessageByUserIdAndGroupId(Guid groupid, Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action, to check draft is exist by user id.
                        List<Domain.Myfashion.Domain.Drafts> lst = session.CreateQuery("From Drafts where GroupId = :groupid and UserId=:userid")
                                           .SetParameter("groupid", groupid)
                                           .SetParameter("userid", UserId)
                                           .List<Domain.Myfashion.Domain.Drafts>()
                                           .ToList<Domain.Myfashion.Domain.Drafts>();
