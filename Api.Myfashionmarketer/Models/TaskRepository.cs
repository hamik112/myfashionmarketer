...
namespace Api.Myfashionmarketer.Models
{
    public class TaskRepository : ITaskRepository
    {
        ...
        /// <param name="UserId"></param>
        /// <returns>Return values in the form of array list.(ArrayList)</returns>
        public List<Domain.Myfashion.Domain.Tasks> getAllTasksOfUserList(Guid UserId, Guid GroupId)
        {
            List<Domain.Myfashion.Domain.Tasks> alstTask = new List<Domain.Myfashion.Domain.Tasks>();
            try
            {
                //Creates a database connection and opens up a session
                using (NHibernate.ISession session = SessionFactory.GetNewSession())
                {
                    ...
                    using (NHibernate.ITransaction transaction = session.BeginTransaction())
                    {
                        ...
                        foreach (var item in query.Enumerable())
                        {
                            alstTask.Add((Domain.Myfashion.Domain.Tasks)item);
                        }
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        for (int i = 0; i < 12; i++)
                        {
                            ...
                            List<Domain.Myfashion.Domain.Tasks> lstFacebookStats = session.CreateQuery("from Tasks where AssignDate Like :year and UserId= :userId ")
                                .SetParameter("userId", Userid)
                                .SetParameter("year", "%" + year + "%", TypeFactory.GetAnsiStringType(15)).List<Domain.Myfashion.Domain.Tasks>().ToList<Domain.Myfashion.Domain.Tasks>();
        ...
        /// <param name="TaskId">Task id.(Guid)</param>
        /// <returns>Return Array list with value.(ArrayList)</returns>
        public List<Domain.Myfashion.Domain.TaskComment> getAllTasksCommentOfUserList(Guid TaskId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    List<Domain.Myfashion.Domain.TaskComment> alstTaskcomment = new List<Domain.Myfashion.Domain.TaskComment>();

                    foreach (var item in query.Enumerable())
                    {
                        alstTaskcomment.Add((Domain.Myfashion.Domain.TaskComment)item);
                    }
                    return alstTaskcomment;
        ...
        public List<Domain.Myfashion.Domain.Tasks> getAllTasksOfUserBYDays(Guid Userid, Guid GroupId,int days)
        {
            List<Domain.Myfashion.Domain.Tasks> alstTask = new List<Domain.Myfashion.Domain.Tasks>();
            try
            {
