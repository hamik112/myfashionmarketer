...
namespace Api.Myfashionmarketer.Models
{
    public class LinkedInAccountRepository : ILinkedInAccountRepository
    {
        ...
        public void addLinkedinUser(Domain.Myfashion.Domain.LinkedInAccount linkedInAccount)
        {
        ...
        public void updateLinkedinUser(Domain.Myfashion.Domain.LinkedInAccount liaccount)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
        ...
        public List<Domain.Myfashion.Domain.LinkedInAccount> GetAllLinkedinAccountsOfUser(Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    NHibernate.IQuery query = session.CreateQuery("from LinkedInAccount where UserId = :userid");
                    query.SetParameter("userid", UserId);
                    List<Domain.Myfashion.Domain.LinkedInAccount> alstLIAccounts = new List<Domain.Myfashion.Domain.LinkedInAccount>();

                    foreach (var item in query.Enumerable())
                    {
                        alstLIAccounts.Add((Domain.Myfashion.Domain.LinkedInAccount)item);
                    }
                    return alstLIAccounts;
        ...
        public List<Domain.Myfashion.Domain.LinkedInAccount> getAllAccountDetail(string profileid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.LinkedInAccount> alst = session.CreateQuery(str)
                       .List<Domain.Myfashion.Domain.LinkedInAccount>()
                       .ToList<Domain.Myfashion.Domain.LinkedInAccount>();
                        return alst;
        ...
        public Domain.Myfashion.Domain.LinkedInAccount getUserInformation(string liuserid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    {
                        //Proceed action, to get details of account.
                        List<Domain.Myfashion.Domain.LinkedInAccount> objlst = session.CreateQuery("from LinkedInAccount where LinkedinUserId = :LinkedinUserId ")                    
                        .SetParameter("LinkedinUserId", liuserid)
                        .List<Domain.Myfashion.Domain.LinkedInAccount>().ToList<Domain.Myfashion.Domain.LinkedInAccount>();
                        Domain.Myfashion.Domain.LinkedInAccount result = new Domain.Myfashion.Domain.LinkedInAccount();
                        if (objlst.Count > 0)
                        {
                            result = objlst[0];
