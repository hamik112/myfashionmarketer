...
namespace Api.Socioboard.Model
{
    public class LinkedinPagePostRepository
    {
        ...
        public List<Domain.Myfashion.Domain.LinkedinCompanyPagePosts> getAllLinkedInPagePostsOfUser(Guid UserId, string pageid)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //After Session creation, start Transaction. 
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.LinkedinCompanyPagePosts> alst = session.CreateQuery("from LinkedinCompanyPagePosts where UserId = :userid and PageId = :pageid ORDER BY PostDate DESC")
                        .SetParameter("userid", UserId)
                        .SetParameter("pageid", pageid)
                        .List<Domain.Myfashion.Domain.LinkedinCompanyPagePosts>()
                        .ToList<Domain.Myfashion.Domain.LinkedinCompanyPagePosts>();
                        return alst;
        ...
        public List<Domain.Myfashion.Domain.LinkedinCompanyPagePosts> getAllLinkedInPostOfPage(string PageId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.LinkedinCompanyPagePosts> alst = session.CreateQuery("from LinkedinCompanyPagePosts where PageId = :pageid ORDER BY EntryDate DESC")
                        .SetParameter("pageid", PageId)
                        .List<Domain.Myfashion.Domain.LinkedinCompanyPagePosts>()
                        .ToList<Domain.Myfashion.Domain.LinkedinCompanyPagePosts>();
