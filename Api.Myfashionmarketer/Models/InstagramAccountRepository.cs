...
namespace Api.Myfashionmarketer.Models
{
    public class InstagramAccountRepository : IInstagramAccountRepository
    {
        ...
        public void addInstagramUser(Domain.Myfashion.Domain.InstagramAccount insaccount)
        {
        ...
        /// </summary>
        /// <param name="insaccount">Set Values in a InstagramAccount Class Property and Pass the same Object of InstagramAccount Class.(Domain.InstagramAccount)</param>
        public void updateInstagramUser(Domain.Myfashion.Domain.InstagramAccount insaccount)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
        ...
        public List<Domain.Myfashion.Domain.InstagramAccount> GetAllInstagramAccountsOfUser(Guid UserId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    NHibernate.IQuery query = session.CreateQuery("from InstagramAccount where UserId = :userid");
                    query.SetParameter("userid", UserId);
                    List<Domain.Myfashion.Domain.InstagramAccount> alstLIAccounts = new List<Domain.Myfashion.Domain.InstagramAccount>();

                    foreach (var item in query.Enumerable())
                    {
                        alstLIAccounts.Add((Domain.Myfashion.Domain.InstagramAccount)item);
                    }
        ...
        public Domain.Myfashion.Domain.InstagramAccount getInstagramAccountDetailsById(string Insuserid, Guid userId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    NHibernate.IQuery query = session.CreateQuery("from InstagramAccount where  InstagramId = :InstagramId and UserId = :UserId")
                     .SetParameter("InstagramId", Insuserid)
                     .SetParameter("UserId", userId);
                    Domain.Myfashion.Domain.InstagramAccount result = (Domain.Myfashion.Domain.InstagramAccount)query.UniqueResult();
                    return result;
                }//End Transaction
            }//End session
        }

        public Domain.Myfashion.Domain.InstagramAccount getInstagramAccountDetailsById(string Insuserid)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    //Proceed action to get Instagram Account Details.
                    List<Domain.Myfashion.Domain.InstagramAccount> objlst = session.CreateQuery("from InstagramAccount where  InstagramId = :InstagramId ")
                     .SetParameter("InstagramId", Insuserid)
                    .List<Domain.Myfashion.Domain.InstagramAccount>().ToList<Domain.Myfashion.Domain.InstagramAccount>();
                    Domain.Myfashion.Domain.InstagramAccount result = new Domain.Myfashion.Domain.InstagramAccount();
                    if (objlst.Count > 0)
                    {
                        result = objlst[0];
                    }
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        bool exist = session.Query<Domain.Myfashion.Domain.InstagramAccount>()
        ...
        public Domain.Myfashion.Domain.InstagramAccount getInstagramAccountById(string InsId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        Domain.Myfashion.Domain.InstagramAccount insAccount = new Domain.Myfashion.Domain.InstagramAccount();
                        foreach (Domain.Myfashion.Domain.InstagramAccount item in query.Enumerable<Domain.Myfashion.Domain.InstagramAccount>())
                        {
                            insAccount = item;
                            break;
                        }
