...
namespace Api.Myfashionmarketer.Models
{
    ...
    {
        public static void Add(Domain.Myfashion.Domain.TumblrAccount tumbler)
        {
        ...
        public bool checkTubmlrUserExists(Domain.Myfashion.Domain.TumblrAccount objTumblrAccount)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
        ...
        public Domain.Myfashion.Domain.TumblrAccount getTumblrAccountDetailsById(string tumblruserid, Guid userId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    Domain.Myfashion.Domain.TumblrAccount result = (Domain.Myfashion.Domain.TumblrAccount)query.UniqueResult();
                    return result;
                }//End Transaction
            }//End session
        }

        public Domain.Myfashion.Domain.TumblrAccount getTumblrAccountDetailsById(string tumblruserid)
        {

            Domain.Myfashion.Domain.TumblrAccount result = new Domain.Myfashion.Domain.TumblrAccount();
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //After Session creation, start Transaction.
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    // proceed action, to get all Facebook Account of User by UserId(Guid) and FbUserId(string).
                    List<Domain.Myfashion.Domain.TumblrAccount> objlsttumb = session.CreateQuery("from TumblrAccount where tblrUserName = :tblruname")
                            .SetParameter("tblruname", tumblruserid)
                       .List<Domain.Myfashion.Domain.TumblrAccount>().ToList<Domain.Myfashion.Domain.TumblrAccount>();
        ...
        public List<Domain.Myfashion.Domain.TumblrAccount> getTumblrAccountDetailsById(Guid userId)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.TumblrAccount> lstmsg = session.CreateQuery("from TumblrAccount where UserId=:userId")
                        .SetParameter("userId", userId)
                        .List<Domain.Myfashion.Domain.TumblrAccount>()
                        .ToList<Domain.Myfashion.Domain.TumblrAccount>();
                        return lstmsg;
                    }
                    catch (Exception ex)
                    {
        ...
        public void updateTumblrUser(Domain.Myfashion.Domain.TumblrAccount tumblrAccount)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
