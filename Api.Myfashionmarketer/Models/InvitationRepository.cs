...
namespace Api.Myfashionmarketer.Models
{
    public class InvitationRepository
    {
        ...
        public void Add(Domain.Myfashion.Domain.Invitation invitation)
        {
            try
            {
        ...
        public bool IsFriendAlreadydInvited(Guid UserId, string FriendEmail)
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<Domain.Myfashion.Domain.Invitation> query = session.CreateQuery("from Invitation where SenderUserId=:UserId and FriendEmail=:FriendEmail")
                                        .SetParameter("UserId", UserId)
                                        .SetParameter("FriendEmail", FriendEmail)
                                        .List<Domain.Myfashion.Domain.Invitation>().ToList<Domain.Myfashion.Domain.Invitation>();
        ...
        public Domain.Myfashion.Domain.Invitation GetInvitationInfoBycode(string code)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //After Session creation, start Transaction. 
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        Domain.Myfashion.Domain.Invitation _Invitation = (Domain.Myfashion.Domain.Invitation)query.UniqueResult();
                        return _Invitation;
                    }
                    catch (Exception ex)
                    {
        ...
        public List<Domain.Myfashion.Domain.Invitation> GetAllInvitedDataOfUser(Guid userid)
        { 
             using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //After Session creation, start Transaction. 
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<Domain.Myfashion.Domain.Invitation> lstInvite = session.CreateQuery("from Invitation where SenderUserId=:Id")
        ...
        public List<Domain.Myfashion.Domain.Invitation> GetInvitedDataOfAcceptedUser(Guid userid)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //After Session creation, start Transaction. 
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<Domain.Myfashion.Domain.Invitation> lstInvite = session.CreateQuery("from Invitation where SenderUserId=:Id and Status=1")
                            .SetParameter("Id", userid)
                            .List<Domain.Myfashion.Domain.Invitation>().ToList<Domain.Myfashion.Domain.Invitation>();
                        return lstInvite;
                    }
        ...
        public Domain.Myfashion.Domain.Invitation UserInvitedInfo(Guid UserId)
        { 
              using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                //After Session creation, start Transaction. 
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        NHibernate.IQuery query = session.CreateQuery("from Invitation where FriendUserId=:FriendUserId and Status=1")
                            .SetParameter("FriendUserId", UserId);
                        return (Domain.Myfashion.Domain.Invitation)query.UniqueResult();
                    }
                    catch (Exception ex)
                    {
                        return null;
