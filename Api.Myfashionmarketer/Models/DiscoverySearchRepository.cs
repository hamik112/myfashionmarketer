...
namespace Api.Myfashionmarketer.Models
{
    public class DiscoverySearchRepository : IDiscoverySearchRepository
    {
        ...
        public void addNewSearchResult(Domain.Myfashion.Domain.DiscoverySearch dis)
        {
        ...
        public List<string> getAllSearchKeywords(Guid Userid)
        {
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    ...
                    List<string> searchresults = session.Query<Domain.Myfashion.Domain.DiscoverySearch>().Where(x => x.UserId == Userid).Select(x => x.SearchKeyword).Distinct().ToList();
        ...
        public void updateNewSearchResult(Domain.Myfashion.Domain.DiscoverySearch dis)
        {
            throw new NotImplementedException();
        }
        public void deleteSearchResult(Domain.Myfashion.Domain.DiscoverySearch dis)
        {
            throw new NotImplementedException();
        }
        ...
        public List<Domain.Myfashion.Domain.DiscoverySearch> getResultsFromKeyword(string keyword)
        {
            //Creates a database connection and opens up a session
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.DiscoverySearch> lst = session.CreateQuery("from DiscoverySearch where SearchKeyword = :keyword")
                         .SetParameter("keyword", keyword)
                          .List<Domain.Myfashion.Domain.DiscoverySearch>()
                          .ToList<Domain.Myfashion.Domain.DiscoverySearch>();
            ...
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                ...
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        ...
                        List<Domain.Myfashion.Domain.DiscoverySearch> lst = new List<Domain.Myfashion.Domain.DiscoverySearch>();

                        foreach (Domain.Myfashion.Domain.DiscoverySearch item in query.Enumerable())
                        {
                            lst.Add(item);
                        }
        ...
        public List<Domain.Myfashion.Domain.DiscoverySearch> GetAllSearchKeywordsByUserId(Guid Userid, string keyword, string network)
        {
            using (NHibernate.ISession session = SessionFactory.GetNewSession())
            {
                using (NHibernate.ITransaction transaction = session.BeginTransaction())
                {
                    try
                    {
                        List<Domain.Myfashion.Domain.DiscoverySearch> lst = session.CreateQuery("from DiscoverySearch where SearchKeyword=:keyword and UserId=:uid and Network=:network")
